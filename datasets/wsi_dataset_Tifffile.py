...
class Wsi_Region(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        region_width, region_height = self.patch_size
        region_left, region_top = tuple(coord)
        dsp_width, dsp_height = self.wsi.level_downsamples[self.level]
        region_left = int(region_left//dsp_width)
        region_top = int(region_top//dsp_height)          

        try:                
            patch = self.wsi.wsi_zarr[self.level][region_top:(region_top+region_height), region_left:(region_left+region_width), :]
            # patch = dask.array.from_zarr(self.wsi.wsi, self.level)[region_top:(region_top+region_height), region_left:(region_left+region_width), :].compute()
        except:                        
            # patch = np.zeros((region_height, region_width, 3))
            raise ValueError
