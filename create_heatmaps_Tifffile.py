...
from vis_utils.heatmap_utils_Tifffile import initialize_wsi, drawHeatmap, compute_from_patches
...
if __name__ == '__main__':
	...
	for i in range(len(process_stack)):
		...
		process_list_filename = os.path.basename(data_args.process_list).replace('.csv', '')
		if data_args.process_list is not None:
			process_stack.to_csv('heatmaps/results/{}.csv'.format(process_list_filename), index=False)
		else:
		...
		for sample in samples:
			if sample['sample']:
				...
				for idx, (s_coord, s_score) in enumerate(zip(sample_results['sampled_coords'], sample_results['sampled_scores'])):
					...
					region_width, region_height = patch_args.patch_size, patch_args.patch_size
					region_left, region_top = tuple(s_coord)
					
					dsp_width, dsp_height = wsi_object.level_downsamples[patch_args.patch_level]
					region_left = int(region_left//dsp_width)
					region_top = int(region_top//dsp_height)   	
					
					patch = Image.fromarray(wsi_object.wsi_zarr[patch_args.patch_level][region_top:(region_top+region_height), \
